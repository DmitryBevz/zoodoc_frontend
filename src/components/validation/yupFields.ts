...
export const phoneNumberField = yup.string().nullable().notRequired().when('phoneNumberField', {
  is: (value: string) => value?.length,
  then: rule => rule.matches(phoneNumberOption, "Будь ласка введіть Ваш номер телефону"),
});
export const passwordField = yup.string().matches(passwordOption, "Введіть Ваш пароль").min(8, "Мінімум 8 символів").required("Будь ласка введіть пароль");
